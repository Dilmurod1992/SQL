...
SELECT *,
       Substring(Vals, 1, Charindex('.', Vals)-1) AS val1,
       Substring(Vals, Charindex('.', Vals)+1, Len(Vals)) AS val2
FROM   Splitter




declare @digit varchar(50)='1234ABC123456XYZ1234567890ADS'
...
select emp2.Name as emp_name, emp2.Salary from Employee as emp1
...
declare @word varchar(50)='rtcfvty34redt'
SELECT CASE
         WHEN @word LIKE '% %' THEN LEFT(@word, Charindex(' ', @word) - 1)
         ELSE name
       END,
       CASE
         WHEN @word LIKE '% %' THEN RIGHT(@word, Charindex(' ', Reverse(@word)) - 1)
       END
FROM  @word 
...
select value from string_split('s-d-g-f-h-s-d-g-f-h-s-@-1-2-1-3-1-3-1-3-1', '-')
...
SELECT 
    Vals,
    LEFT(Vals, 
         PATINDEX('%[^0-9]%', Vals + 'A') - 1) AS LeadingInteger
FROM GetIntegers
WHERE Vals LIKE '[0-9]%';
...
SELECT 
    ...
    Vals AS OriginalVals,
       ...
       STUFF(
        STUFF(Vals, 1, LEN(FirstVal), SecondVal),
        LEN(SecondVal) + 1 + 1,  -- +1 for comma, +1 for shifting
        LEN(SecondVal),
        FirstVal
    ) AS SwappedVals
