...
select c.FirstName, c.LastName, ord.OrderDate from Customers as c
...
select ord.OrderID, ord.OrderDate, p.PaymentDate, p.Amount from Orders ord
...
select a.Name as emp_name, b.Name as manager_name from Employees as a
...
select st.Name, co.CourseName from Students as st
...
select c.FirstName, c.LastName, sum(o.Quantity) as total from Customers as c
...
select c.FirstName, c.LastName, o.Quantity as total from Customers as c
inner join Orders as o
on c.CustomerID=o.CustomerID
where o.Quantity>3
...
select dep.DepartmentName, count(emp.Name) as num_of_employees from Employees as emp
inner join Departments as dep
on emp.DepartmentID=dep.DepartmentID
...
select p.ProductName, s.ProductID, s.SaleDate from Products as p
...
select c.FirstName, c.LastName, sum(ord.Quantity) as total_orders from Customers as c
inner join Orders as ord
on c.CustomerID=ord.CustomerID
group by c.FirstName, c.LastName
...
on emp.DepartmentID=dep.DepartmentID
where dep.DepartmentName is not null and emp.Name is not null 
...
select a.Name as emp_name1, b.Name as emp_name2, a.ManagerID from Employees as a
inner join Employees as b
on a.EmployeeID=b.ManagerID
...
select ord.OrderID, ord.OrderDate, c.FirstName, c.LastName from Orders ord
...
full outer join Payments as pay
on ord.OrderID=pay.OrderID
where ord.OrderDate is not null and pay.PaymentDate is not null
...
select ord.ProductID, p.ProductName from Products as p
...
select a.DepartmentID, avg(a.Salary)as avg_salary from Employees as a
inner join Employees as b
on a.EmployeeID=b.EmployeeID
group by a.DepartmentID, b.Salary
having b.Salary>avg(a.Salary)
...
select ord.OrderDate, pay.OrderID, pay.PaymentDate from Orders as ord
left join Payments as pay
on ord.OrderID=pay.OrderID
where year(ord.OrderDate)<2022 and pay.PaymentDate is null
...
select a.Name as emp_name1, b.Name as emp_name2, a.ManagerID, a.Salary from Employees as a
inner join Employees as b
on a.EmployeeID=b.ManagerID
where a.ManagerID is not null and a.Salary>60000
...
select st.StudentID, st.Name, co.CourseName from Students as st
inner join Enrollments as en
on st.StudentID=en.StudentID
inner join Courses as co
on en.CourseID=co.CourseID
...
select pay.OrderID, ord.OrderDate, pay.PaymentID, pay.PaymentDate from Orders as ord
left join Payments as pay
on ord.OrderID=pay.OrderID
where pay.PaymentDate is null
