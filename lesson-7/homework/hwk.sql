...
select count(EmployeeID) as numb_of_employees, DepartmentName
from Employees
...
select CustomerID, sum(SaleAmount) as total_sale_per_cust
from Sales
group by CustomerID
...
select DepartmentName, count(EmployeeID) as num_of_emp
from Employees
group by DepartmentName
...
select ProductID as product_category, sum(SaleAmount) as total_sale, avg(SaleAmount) as avg_sale
from Sales
group by ProductID
...
select DepartmentName, avg(Salary) as average_salary, count(*) as num_of_emp
from Employees
group by DepartmentName
...
select Category, avg(Price) as agv_price
from Products
group by Category
having avg(Price)>40
...
select SaleDate, sum(SaleAmount) as total_sale_amount
from Sales
group by SaleDate
...
select CustomerID, sum(Quantity) as num_of_orders
from Orders
group by CustomerID
...
select CustomerID, sum(SaleAmount) as total_sale_amount
from Sales
group by CustomerID
...
select OrderID, sum(TotalAmount) as total_sale_amount
from Orders
group by OrderID
