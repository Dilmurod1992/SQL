...
select emp.name , emp.salary, man.name as manager_name, man.salary  from Employee as emp
...
select per.email --count(p.email) as per_email 
from Person1 as per
join Person1 as p
on per.id=p.id
group by per.email
...
create table Person2(id int, email varchar(50))
...
DELETE p1 
FROM Person2 as p1 
JOIN Person2 as  p2 ON p1.Email = p2.Email 
WHERE p1.Id > p2.Id;
