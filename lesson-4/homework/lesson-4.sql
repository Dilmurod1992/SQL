...
select * from Products
WHERE Price< select AVG(Price)*0.10


SELECT AVG(Price)
FROM Products
WHERE Price=AVG(Price)*0.10
...
SELECT * FROM Employees 
WHERE Salary >= ALL(Select Salary from Employees where DepartmentName='IT')
...
SELECT * FROM  Orders
WHERE DATEDIFF(day,OrderDate,GETDATE()) between 0 and 180