...
select PHONE_NUMBER, replace(PHONE_NUMBER, 124, 199) from Employees




select FIRST_NAME, len(FIRST_NAME) as len_of_f_name from Employees
...
SELECT COALESCE(NULL, NULL, NULL, ssn,null, passportid, null, itin) from person
...
select DEPARTMENT_ID, COUNT(FIRST_NAME) as num_of_employees
from Employees
...
select JobDescription, max(MissionCount) as most_experienced, min(MissionCount) as least_experienced
from Personal
...
(SELECT SUM(Grade) FROM Students as a WHERE a.StudentID  = b.StudentID) as Sum_of_previous_row FROM Students as b;
...
select a.StudentName, b.StudentName, a.Birthday, b.Birthday from Student as a
join Student as b
...
Select P1.PlayerA, P2.PlayerB, sum(P1.Score) as TotalScore  from PlayerScores as P1 
  Join PlayerScores as P2 on P1.PlayerA = P2.PlayerB
 group by P1.PlayerA, P2.PlayerB
