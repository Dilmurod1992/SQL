...
select ord.OrderID, c.FirstName, ord.OrderDate from Orders as ord
join Customers as c
...
select dep.DepartmentName, max(emp.Salary) as max_salary from Employees as emp
join Departments as dep
on emp.DepartmentID=dep.DepartmentID
...
select c.FirstName, c.LastName, ord.OrderID, ord.OrderDate from Customers as c
join Orders as ord
...
select c.FirstName, c.LastName, sum(ord.Quantity) as total_order from Customers as c
join Orders as ord
on c.CustomerID=ord.CustomerID
...
select c.FirstName, sum(ord.Quantity) as total_order from Customers as c
join Orders as ord
on c.CustomerID=ord.CustomerID
...
select pr.ProductName, sup.SupplierName from Products as pr
join Suppliers sup
on pr.SupplierID=sup.SupplierID 
where sup.SupplierName='Gadget Supplies' or sup.SupplierName='Clothing Mart'
...
select c.FirstName, max(ord.OrderDate) as most_recent_order_date from Customers as c
...
select c.FirstName, sum(ord.TotalAmount) as total_order from Customers as c
join Orders as ord
on c.CustomerID=ord.CustomerID
group by c.FirstName
...
select pro.ProductName, s.SaleDate, s.SaleAmount from Products as pro
join Sales as s
...
select pro.ProductName, sum(s.SaleAmount) as total_sale_amount from Products as pro
join Sales as s
on pro.ProductID=s.ProductID
...
select pro.ProductName, s.SaleDate, pro.StockQuantity from Products as pro
join Sales as s
on pro.ProductID=s.ProductID
where Year(s.SaleDate)=2023 and pro.StockQuantity>100
...
select c.FirstName, ord.OrderID, ord.OrderDate, c.Address, c.Country from Customers as c
join Orders as ord
on c.CustomerID=ord.CustomerID
...
select pro.ProductName, pro.Category, s.SaleAmount from Products as pro
join Sales as s
on pro.ProductID=s.ProductID
where pro.Category='Electronics' or s.SaleAmount>350
...
select c.CategoryName, count(pro.ProductName) as num_of_products from Products as pro
join Categories c
on pro.Category=c.CategoryName
...
select c.FirstName, c.City, ord.OrderID, ord.TotalAmount from Customers as c
join Orders as ord
on c.CustomerID=ord.CustomerID
where c.City='Los Angeles' and ord.TotalAmount>300
...
join Departments as dep
on emp.DepartmentID=dep.DepartmentID
where (dep.DepartmentName='Human Resources' or dep.DepartmentName='Finance') or (emp.Name like '%[a,i,e,o,u]%[a,i,e,o,u]%[a,i,e,o,u]%[a,i,e,o,u]%')
