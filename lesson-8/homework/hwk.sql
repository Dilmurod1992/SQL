...
select Category, count(ProductName) as amount
from Products
group by Category
...
case when StockQuantity<30 then 'Low Stock'
	 else 'Sufficient' end as stock_label
from Products
...
select Country, count(FirstName) as num_of_customers
from Customers
group by Country
...
where InvoiceDate like '2023-01%'
union
...
SELECT 
    YEAR(OrderDate) AS order_year, Month(OrderDate) AS order_month,
...
case when Price<100 then 'Low'
	 ...
	 else 'High' end as price_level
from Products
group by Price, ProductName
order by price desc
...
select ProductID, sum(SaleAmount) as sale_per_product
from Sales
group by ProductID
...
(
 ...
 select* from Population_Each_City2
...
select top 3 with ties CustomerID, sum(TotalAmount) as total_spent 
from Invoices
group by CustomerID
...
select  district_name, population, year from Population_Each_City2
unpivot(
